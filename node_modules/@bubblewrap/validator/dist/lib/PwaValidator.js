...
exports.PwaValidator = void 0;
const psi_1 = require("./psi");
...
class PwaValidator {
     ...
     */
    async validate(url) {
        const psiRequest = new psi_1.PsiRequestBuilder(url)
        ...
        const fidStatus = this.getStatus(fidScore, MIN_FID_PASS_SCORE, MIN_FID_WARN_SCORE);
    ...
    static async validate(url) {
        const validator = new PwaValidator(new psi_1.PageSpeedInsights());
        return await validator.validate(url);
    }
    getStatus(value, minPassScore, minWarnScore) {
        return value <= minPassScore ? 'PASS' : value <= minWarnScore ? 'WARN' : 'FAIL';
    }
}
exports.PwaValidator = PwaValidator;
